...
function parallax_enqueue_scripts_styles() {
	...
	wp_enqueue_style( 'custom', get_stylesheet_directory_uri() . '/css/allstyles.css?v=1', array() );
	wp_enqueue_style( 'icomoon-fonts', get_stylesheet_directory_uri() . '/icomoon.css', array() );
	wp_enqueue_style( 'googlefonts', '//fonts.googleapis.com/css?family=Montserrat:300,400,600,700', array() );
	...
	if (!isset($_GET['fl_builder'])) {
		// wp_enqueue_script( 'SpeedBump', get_bloginfo( 'stylesheet_directory' ) . '/js/speedbump.js', '1.0.0' );
		//wp_enqueue_script( 'jquery-for-speed-bump', '//code.jquery.com/jquery-1.9.1.js', '1.9.1' );
	}
	*/
	wp_enqueue_script( 'scripts', get_stylesheet_directory_uri() . '/js/scripts.js?v=5', array() );
	
}


// ShiftNav plugin interfering with Page Builder - GW - 03-006-23 Temp workaround!
function dequeue_shiftnav_inadmin(){
    if( current_user_can('administrator') ) {  
    	wp_dequeue_style( 'shiftnav-js' ); 
    	wp_dequeue_style( 'shiftnav' ); 
	}
}  
add_action( 'wp_enqueue_scripts', 'dequeue_shiftnav_inadmin', 9999 );
...
function global_footer(){
	...
	// New social media icon footer
	echo do_shortcode('[fl_builder_insert_layout slug="global-footer-copy"]');
}
...
genesis_register_sidebar( array(
	'id'			=> 'header-links',
	'name'			=> __( 'Header Links', 'thrive' ),
	'description'	=> __( 'This is the top header area.', 'thrive' ),
) );
...
function header_widget() {
	if (is_active_sidebar( 'header-links' ) ) {
 	genesis_widget_area( 'header-links', array(
		'before' => '<div class="header-links"><div class="wrap">',
		'after'  => '</div></div>',
	) );
}}
	...
	global $blogurl;
 	echo '<div class="clearboth copy-line">
 			<div class="copyright first">
 				<p><span id="copy">Copyright &copy; 2024 - All rights reserved</span> <span class="format-pipe">&#124;</span>  
...
add_image_size( 'news-thumb', 250, 167, false );
add_image_size( 'news-full', 800, 300, false );
add_image_size( 'home-blog', 340, 178, false );
...
function mscu_smart_app_banner() {
	wp_enqueue_script( 'jquery-ultimate-smartbanner', get_stylesheet_directory_uri() . '/js/jquery-ultimate-smartbanner.js', array( 'jquery' ) );
}
add_action( 'wp_enqueue_scripts', 'mscu_smart_app_banner' );